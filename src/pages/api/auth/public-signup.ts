 ...
 * Umożliwia samodzielne założenie konta przez pacjenta.
 *
 * Request Body (JSON):
 * - email: string (wymagany, format email)
 * - password: string (wymagany, SHA-256 hash 64 znaków)
 * - firstName: string (wymagany)
 * - lastName: string (wymagany)
 * - age: number (opcjonalny)
 * - gender: 'male' | 'female' (opcjonalny)
...
export const POST: APIRoute = async ({ request, cookies }) => {
  try {
    ...
    // 2. Validate input with Zod (invitationToken jest opcjonalne)
    ...
    // authService automatycznie wykrywa brak invitationToken i traktuje jako publiczną rejestrację
